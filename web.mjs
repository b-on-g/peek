...
;
	($.$bog_peek_app) = class $bog_peek_app extends ($.$mol_page) {
		Source(){
			const obj = new this.$.$mol_link_source();
			(obj.uri) = () => ("https://github.com/b-on-g/peek");
			return obj;
		}
		Theme_toggle(){
		...
		Hero_title(){
			const obj = new this.$.$mol_text();
			(obj.text) = () => ("# Список диалогов ВК, в котором видно, о чём речь");
			return obj;
		}
		Hero_lead(){
			const obj = new this.$.$mol_text();
			(obj.text) = () => ("ВКонтакте показывает одну обрезанную строку последнего сообщения. Peek разворачивает превью: больше строк текста и несколько последних сообщений сразу. Заходить в диалог, чтобы понять, о чём он, больше не нужно.");
			return obj;
		}
		Download(){
			const obj = new this.$.$mol_link();
			(obj.uri) = () => ("peek.zip");
			(obj.sub) = () => (["Скачать расширение"]);
			return obj;
		}
		Repo(){
			const obj = new this.$.$mol_link();
			(obj.uri) = () => ("https://github.com/b-on-g/peek");
			(obj.sub) = () => (["Исходники на GitHub"]);
			return obj;
		}
		Hero_actions(){
			const obj = new this.$.$mol_row();
			(obj.sub) = () => ([(this.Download()), (this.Repo())]);
			return obj;
		}
		Hero(){
			const obj = new this.$.$mol_list();
		...
		Before_name(){
			const obj = new this.$.$mol_view();
			(obj.sub) = () => (["Леонид Фирстов"]);
			return obj;
		}
		Before_text(){
			const obj = new this.$.$mol_view();
			(obj.sub) = () => (["по договору всё согласовали, отправляю скан на подпись сегодня вечером"]);
			return obj;
		}
		Before_body(){
			const obj = new this.$.$mol_list();
			(obj.rows) = () => ([(this.Before_name()), (this.Before_text())]);
			return obj;
		}
		Before_row(){
			const obj = new this.$.$mol_row();
			(obj.sub) = () => ([(this.Before_ava()), (this.Before_body())]);
			return obj;
		}
		Before(){
			const obj = new this.$.$mol_list();
			(obj.rows) = () => ([(this.Before_cap()), (this.Before_row())]);
			return obj;
		}
		After_cap(){
			const obj = new this.$.$mol_view();
		...
		After_text_1(){
			const obj = new this.$.$mol_view();
			(obj.sub) = () => (["Я: скинул договор, посмотрите когда будет минута"]);
			return obj;
		}
		After_text_2(){
			const obj = new this.$.$mol_view();
		...
		After_row(){
			const obj = new this.$.$mol_row();
			(obj.sub) = () => ([(this.After_ava()), (this.After_body())]);
			return obj;
		}
		After(){
			const obj = new this.$.$mol_list();
			(obj.rows) = () => ([(this.After_cap()), (this.After_row())]);
			return obj;
		}
		Demo(){
			const obj = new this.$.$mol_row();
			(obj.sub) = () => ([(this.Before()), (this.After())]);
			return obj;
		}
		Install(){
			const obj = new this.$.$mol_text();
			(obj.text) = () => ("## Как поставить\n\nРасширения пока нет в Chrome Web Store, поэтому ставится вручную. Это три минуты и делается один раз.\n\n1. Скачайте архив и распакуйте его в любую папку, которую не будете удалять.\n2. Откройте `chrome://extensions` и включите «Режим разработчика» в правом верхнем углу.\n3. Нажмите «Загрузить распакованное расширение» и укажите распакованную папку.\n4. Откройте [vk.ru/im](https://vk.ru/im) и обновите страницу.\n\nРаботает в Chrome, Arc, Edge, Яндекс.Браузере и других браузерах на Chromium.");
			return obj;
		}
		Options(){
			const obj = new this.$.$mol_text();
			(obj.text) = () => ("## Настройки\n\nКлик по иконке расширения открывает два ползунка.\n\n**Строк** — сколько строк текста показывать в одном сообщении. По умолчанию 5.\n\n**Последних сообщений** — сколько последних сообщений диалога показывать. По умолчанию 2. Поставьте 1, если хватает одного, но хочется видеть его целиком.\n\nОбщий тумблер выключает расширение и возвращает штатный вид ВК, не удаляя его.");
			return obj;
		}
		Privacy(){
			const obj = new this.$.$mol_text();
			(obj.text) = () => ("## Что с данными\n\nНичего никуда не уходит. Расширение работает только на страницах ВК, настройки лежат в браузере, сервера у проекта нет. Прошлые сообщения берутся тем же способом, каким их берёт сам сайт ВК, и дальше вкладки не уезжают.");
			return obj;
		}
		Next(){
			const obj = new this.$.$mol_text();
			(obj.text) = () => ("## Что дальше\n\nСледующий шаг — ответ прямо из списка диалогов, не открывая переписку. Пока описан [в PRD](https://github.com/b-on-g/peek/blob/main/PRD.md), в коде ещё нет.");
			return obj;
		}
		Content(){
			const obj = new this.$.$mol_list();
		...
		tools(){
			return [(this.Source()), (this.Theme_toggle())];
		}
		plugins(){
			return [(this.Theme())];
		}
	...
	($mol_mem(($.$bog_peek_app.prototype), "Source"));
	($mol_mem(($.$bog_peek_app.prototype), "Theme_toggle"));
	($mol_mem(($.$bog_peek_app.prototype), "Theme"));
	($mol_mem(($.$bog_peek_app.prototype), "Hero_title"));
	($mol_mem(($.$bog_peek_app.prototype), "Hero_lead"));
	($mol_mem(($.$bog_peek_app.prototype), "Download"));
	($mol_mem(($.$bog_peek_app.prototype), "Repo"));
	($mol_mem(($.$bog_peek_app.prototype), "Hero_actions"));
	($mol_mem(($.$bog_peek_app.prototype), "Hero"));
	($mol_mem(($.$bog_peek_app.prototype), "Before_cap"));
	($mol_mem(($.$bog_peek_app.prototype), "Before_ava"));
	($mol_mem(($.$bog_peek_app.prototype), "Before_name"));
	($mol_mem(($.$bog_peek_app.prototype), "Before_text"));
	($mol_mem(($.$bog_peek_app.prototype), "Before_body"));
	($mol_mem(($.$bog_peek_app.prototype), "Before_row"));
	($mol_mem(($.$bog_peek_app.prototype), "Before"));
	($mol_mem(($.$bog_peek_app.prototype), "After_cap"));
	($mol_mem(($.$bog_peek_app.prototype), "After_ava"));
	($mol_mem(($.$bog_peek_app.prototype), "After_name"));
	($mol_mem(($.$bog_peek_app.prototype), "After_text_1"));
	($mol_mem(($.$bog_peek_app.prototype), "After_text_2"));
	($mol_mem(($.$bog_peek_app.prototype), "After_body"));
	($mol_mem(($.$bog_peek_app.prototype), "After_row"));
	($mol_mem(($.$bog_peek_app.prototype), "After"));
	($mol_mem(($.$bog_peek_app.prototype), "Demo"));
	($mol_mem(($.$bog_peek_app.prototype), "Install"));
	($mol_mem(($.$bog_peek_app.prototype), "Options"));
	($mol_mem(($.$bog_peek_app.prototype), "Privacy"));
	($mol_mem(($.$bog_peek_app.prototype), "Next"));
	($mol_mem(($.$bog_peek_app.prototype), "Content"));
